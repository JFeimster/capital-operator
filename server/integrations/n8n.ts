  ...
  async dispatch(payload: UnifiedLeadInput, context?: Record<string, unknown>): Promise<IntegrationAdapterResult> {
    ...
    if (!this.isEnabled()) {
      return {
        provider: this.id,
        status: 'DISABLED',
        message: 'n8n integration is disabled via environment configuration.',
    ...
    if (!webhookUrl) {
      return {
        provider: this.id,
        status: 'DEGRADED',
        message: 'N8N_WEBHOOK_URL not configured in server environment. Skipping sync.',
        durationMs: Date.now() - startTime
      };
    }
    ...
    try {
      ...
      return {
        provider: this.id,
        status: 'FAILED',
        error: `n8n webhook network exception: ${err?.message || 'Unknown error'}`,
        durationMs: Date.now() - startTime
      };
    }
  }
