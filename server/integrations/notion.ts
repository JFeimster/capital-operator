  ...
  async dispatch(payload: UnifiedLeadInput, context?: Record<string, unknown>): Promise<IntegrationAdapterResult> {
    ...
    if (!this.isEnabled()) {
      return {
        provider: this.id,
        status: 'DISABLED',
        message: 'Notion integration is disabled via environment configuration.',
    ...
    if (!token) {
      return {
        provider: this.id,
        status: 'DEGRADED',
        message: 'NOTION_TOKEN not configured in server environment. Skipping sync.',
        durationMs: Date.now() - startTime
      };
    }
    ...
    try {
      const response = await fetch('https://api.notion.com/v1/pages', {
        ...
        body: JSON.stringify({
          ...
          properties: {
            Name: {
              title: [{ text: { content: `${firstName || 'Lead'} (${company || 'Company'})` } }]
            },
            Email: {
      ...
      if (response.ok) {
        ...
        return {
          ...
          message: 'Notion lead page created successfully.',
          durationMs
        };
      }
      ...
      return {
        ...
        status: 'FAILED',
        statusCode: response.status,
        error: `Notion API error: ${errorText}`,
        durationMs
      };
    } catch (err: any) {
      return {
        provider: this.id,
        status: 'FAILED',
        error: `Notion network exception: ${err?.message || 'Unknown error'}`,
        durationMs: Date.now() - startTime
      };
    }
  }
