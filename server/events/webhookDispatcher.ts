...
import { CapitalEvent, WebhookDeliveryResult } from './types';
...
/**
 * Dispatches a signed CapitalEvent payload to a destination URL.
 * Produces structured WebhookDeliveryResult with timing, response, and error tracking.
 */
export async function dispatchWebhook(
  ...
  try {
    ...
    return {
      ...
      attempt: 1,
      durationMs,
      error: `Destination server returned HTTP ${response.status}`,
      timestamp: new Date().toISOString()
    };
  } catch (err: any) {
    clearTimeout(timeoutId);
    ...
    return {
      ...
      attempt: 1,
      durationMs,
      error: isAbort ? `Webhook dispatch timed out after ${timeoutMs}ms` : (err?.message || 'Network failure'),
      timestamp: new Date().toISOString()
    };
  }
}
